...
export class Client {
     ...
     * @return Success
     */
    registerAdmin(body: RegisterModel | undefined): Observable<void> {
     ...
     * @return Success
     */
    data(): Observable<EmployeeEntity> {
    ...
    protected processData(response: HttpResponseBase): Observable<EmployeeEntity> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
